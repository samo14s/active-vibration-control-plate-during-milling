...
python make_uncertainty_figure.py  # متانة عدم يقين المقال الجمعي W_Pau (المعادلة 19)
...
|:---:|:---:|:---:|
| 1.0× (دون الاسمي) | ✅ 2.35µm / 9.6V | ✅ 3.20µm |
| **1.6× = الشرط الاسمي S²** | ✅ 3.75µm — **بلا أي هامش** | ✅ 5.11µm / 21.7V |
| **1.8×** (+12% فوق الاسمي) | ❌ يتباعد (t≈20.3s) | ✅ **5.75µm / 23V** |
| **2.0×** (+25% فوق الاسمي) | ❌ | ✅ **6.43µm / 39V** |
| 2.2× / 2.5× | ❌ | ❌ (t≈20.0/19.5s) |

¹ القيم RMS **كلّي** للمرور (اصطلاح شكل السقف)؛ `full_pass.py` يستعمل متوسّط RMS
**النافذي** فيبدو أعلى قليلاً (3.71 مقابل 3.75 الكلّي عند 1.6×). ² معايرة النموذج تجعل
شرط الورقة S يقابل عاملَ قوة **1.6×** من المتوسّط (`ALPHA4_NOMINAL_FACTOR`) — أي أن
سقف ATD‑SMC هو الاسمي نفسه **بلا هامش تصعيد**، بينما يمنح AST‑SMC **هامش +25%**.
...
### 4) متانة أمام **عدم يقين المقال الجمعي** $W_{Pau}$ (المعادلة 19، من `make_uncertainty_figure.py`)

اختبار **نموذج عدم اليقين الخاص بالورقة نفسِها**: اضطراب جمعي ديناميكي على مسار
المشغّل→الحسّاس $G_{\text{true}}=G+\Delta W_{Pau}$، $\|\Delta\|_\infty\le1$. مقياسُه
ضئيل عند النمطين المتحكَّم بهما لكنه **يهيمن في القيعان بين‑النمطية والحزام العالي**
(‏2000–3500 Hz، نحو الصفر اليمنى). RMS مستقرّة (µm)، أسوأ Δ، مع مسح مقدار $\kappa$:

| $\kappa\cdot W_{Pau}$ | 0 | 0.25 | 0.5 | 0.75 | 1.0 (كامل الورقة) |
|---|:---:|:---:|:---:|:---:|:---:|
| SMC | 4.9 | 5.1 | 5.5 | ❌ 67 | ❌ **93 / تشبّع** |
| **TD‑SMC** | 3.9 | 4.0 | 4.1 | 4.2 | ✅ **4.3 / 11V** |
| **ATD‑SMC** ⭐ | 4.2 | 4.3 | 4.4 | 4.6 | ✅ **4.8 / 11V** |
| AST‑SMC | 6.8 | 7.4 | 8.7 | ❌ 122 | ❌ **130 / تشبّع** |

**نتيجة حاسمة مسنودة بمعيار الورقة نفسِه** ($\|W_{Pau}\,KS\|_\infty<1$):
- **TD‑SMC وATD‑SMC مسطّحان** عبر كامل مقدار عدم اليقين — حساسية تحكّمهما $KS$ صغيرة في
  حزام $W_{Pau}$ (وصول ناعم + طبقة حدّية واسعة + تنعيم حدّ التأخير) فيحقّقان الحدّ.
- **SMC وAST‑SMC يعبران حدّ الاستقرار** بين $\kappa=0.5$ و$0.75$ (انسكاب طيفي spillover:
  محتواهما الترددي العالي — طبقة ضيّقة / Super‑Twisting — يُثير الأنماط غير المُنمذَجة).
- **يقوّي ATD‑SMC ويكشف AST‑SMC:** ATD‑SMC الآن متينٌ على **أربعة محاور** (قوة، تردّد،
  حدّ ضيّق، **وعدم يقين الورقة الجمعي**)؛ وAST‑SMC هو **الأسوأ** هنا (130µm) — ثمنُ
  Super‑Twisting الذي يمنحه المِظروف المتحرّك يجعله الأهشّ أمام الديناميكا غير المُنمذَجة.
  (التحقّق: خطّ $\delta=0$ يطابق القيم الاسمية المعلومة تماماً، فالفشل فيزيائيّ لا عدديّ.)
...
![عدم يقين المقال](results/fig_uncertainty.png)
...
│   ├── uncertainty.py          # عدم يقين المقال الجمعي W_Pau (المعادلة 19) + محاكيه
