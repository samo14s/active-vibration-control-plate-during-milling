...
DT = 5e-5
SENSOR_FLOOR = 0.0      # capteur fin (le bruit/retard capteur est déjà modélisé
                        # par PiezoActuator dans ce script d'étude piézo réaliste)
T_STOP_NO_CTRL = 0.20
...
print(f"\n=== Pré-calcul coupe non linéaire (alpha3, alpha4, alpha4_2, alpha4_3) ===")
n_per = int(np.round(tau/DT))
alpha3_t, alpha4_t, alpha4_2_t, alpha4_3_t = precompute_nonlinear_periodic(
    DT, n_per, sim.nstep,
