...
| "Stability domain 21.7× open loop, +41 % vs LQG" | `zeta_DARC_eff = zeta_LQG * 1.30` — a hard-coded 30 % damping bonus. The DARC controller object never enters the stability computation. The ratio derived from it is printed in the figure title as a result. |
