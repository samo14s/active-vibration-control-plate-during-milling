...
> اللاخطي بدقة 0.01–1.8 % عند نقاط التحقق المرجعية. WP3 ✅ (جدول الأعماق المعتمدة + حساسية h_req +
...
> آلية Ozsoy. **والاستنساخ الخارجي منجز على المعاملات المنشورة**
> (`avc/sdof.py::OZSOY_PUBLISHED`، بعد توفر PDF الورقة): خريطة القوة
> القسرية تطابق قياسات Fig. 10 بوسيط نموذج/قياس = 1.04، وكلتا جزيرتي
> Fig. 12 تظهران في موضعيهما كمناطق S في إحصاء الشهادات مع الفجوة
> الخالية من الإشباع بينهما — الجزر المنشورة هي إشباع الاستجابة
> القسرية لنمطٍ داخل نطاق توافقيات مرور الأسنان، وهو بالضبط ما يتنبأ
> به رفعُنا مع الارتجاع مضمَّنًا).
> WP5 ✅ ضمن امتداد النظام المشبع: شهادة استرجاع شاملة بمعيار الدائرة
> على حلقة deadzone المرفوعة + بحث خطي لـanti-windup على مستواها
> (`certify_saturated`, `aw_min_gamma`؛ AW مُثبَت أنه لا يغيّر أي شهادة
> خطية). **نتيجة بنيوية:** فوق حد الحلقة المفتوحة لا يمكن لأي شهادة
> قطاع-[0,1] شاملة أن توجد (حافة القطاع تتضمن مشغّلًا ميتًا) —
> الشهادات الإقليمية هناك ضرورة لا تحفظًا. الاختبارات الكافية لا تعتمد
> الشمولية في أي نقطة مقيسة (c≈1.5–13) — طريق التحسين المحدد:
> مضاعِفات Zames–Falb. وإعادة محاكاة SDOF (صنف Ozsoy، بروتوكول سلطة
> مكافئة، خانة معاملات منشورة): لا جزر حتى عمق قص مكافئ D≈4 — التغذية
> السرعية المقصوصة تبقى مبددة؛ العامل الحاسم للجزر هو سلامة طور
> التغذية المقصوصة لا عمق القص (سجل §6 من النص التأسيسي).
...
> الخطية 3.2× عند أسوأ موضع تنهار إلى 1.32× عند تفاوت 1 µm وتنقلب إلى
> 0.79× عند 20 µm (سجل §5 من النص التأسيسي) — وهو، على حدّ علمنا، أول
> برهان كمّي (محاكاةً، على منصة مرسَّخة قياسيًا) على أن هوامش
> الاستقرار الخطية قد تُسيء ترتيب متحكمات التفريز تحت الإشباع.
