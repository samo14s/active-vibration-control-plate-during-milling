...
La tension n'entre pas dans le critere : le simulateur ecrete deja a +/- V_MAX
(cf. F1), donc l'autorite est bornee par construction et un correcteur ne peut
pas "acheter" de la stabilite avec une tension illimitee. La tension RMS au
point optimal est rapportee separement, pour information.
...
def _worst_blim(arch, x, tol, cutoff=0.0):
...
def _eval(job):
    arch, x, tol, cutoff = job
    return _worst_blim(arch, np.asarray(x), tol, cutoff)


def pso(arch, n_part, n_iter, workers, rng):
    ...
    with mp.Pool(workers, initializer=_init_worker) as pool:
        f = np.array(pool.map(_eval, [(arch, x, TOL_SEARCH, 0.0) for x in X]))
        ...
        for it in range(n_iter):
            ...
            X = np.clip(X + V, lo, hi)
            f = np.array(pool.map(
                _eval, [(arch, x, TOL_SEARCH, 0.97*gbest_f) for x in X]))
            imp = f > pbest_f
...
def report(arch, x):
    ...
    for r in SPEEDS:
        tau = 60.0/(3*r)
        res = sim.run(mk(tau/82, tau), rpm=r, ap=0.25e-3, T=SB.T_RUN)
        u.append(res['rms_u'])
    return lim, u
...
def main():
    ...
    for arch in archs:
        ...
        print(f'  tension RMS V : {[round(v, 2) for v in u]}')
