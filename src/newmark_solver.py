...
class NewmarkSimulator:
    ...
    def simulate(self, alpha3_t, alpha4_t,
        ...
        for k in range(1, nstep):
            ...
            # Observer + control command. The step signature is selected by
            # duck-typed capability flags rather than a hard-coded class list:
            #   enable_gs    -> the controller wants the tool position (scheduling)
            #   needs_kstep  -> the controller wants the step index (phase-aware FF)
            if controller is not None:
                if getattr(controller, 'enable_gs', False):
                    x_p_now_phys = self.plate.xp_array[kp]
                    step_out = controller.step(x_hat[:, k-1],
                                                u_real_prev, y_obs_now,
                                                x_p_now=x_p_now_phys)
                elif getattr(controller, 'needs_kstep', False):
                    step_out = controller.step(x_hat[:, k-1],
                                                u_real_prev, y_obs_now,
                                                k_step=k)
                else:
